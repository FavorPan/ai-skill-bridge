...
1. **Strip source-specific frontmatter** — Remove tool-specific YAML metadata (Hermes `trigger`/`tags`, Cursor `globs`/`alwaysApply`).
2. **Inline referenced files** — Merge content from `references/`, `templates/`, `scripts/` into the main file.
3. **Keep core instructions intact** — Design rules, checklists, patterns, workflows transfer as-is.
4. **Adapt delivery hints** — For tools without slash commands (Codex, OpenClaw), add invocation notes.
5. **Verify file size** — Keep under ~15KB for Claude Code commands.
6. **Update registry** — Track ports in a central registry.

## Pitfalls

- Do NOT copy YAML frontmatter verbatim — Claude Code/Codex render it as visible text.
- Codex/OpenClaw have no slash commands — reference files manually or via AGENTS.md/SOUL.md.
- Claude Code: `~/.claude/commands/` = global, `.claude/commands/` = project-only.
- Cursor: project-only, no global equivalent.
